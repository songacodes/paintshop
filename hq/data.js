...
async function restoreClient(branchId, clientId) {
  const response = await fetch(`${API_BASE}/restore/client/${branchId}/${clientId}`, {
    method: 'POST',
    headers: getHeaders()
  });
  if (!response.ok) {
...
const DataAPI = {
    ...
    async deleteClient(branchId, clientId) {
      console.log('[DEBUG] Deleting client:', { branchId, clientId });
      const response = await fetch(`${API_BASE}/clients/${branchId}/${clientId}`, {
        method: 'DELETE',
        headers: getHeaders()
      });
      ...
      if (!response.ok) {
        const error = await response.json();
        console.error('API Error deleting client:', response.status, error);
    ...
    async getAllShops() {
        const response = await fetch(`${API_BASE}/all-shops`);
    ...
    async getAllClients() {
        const response = await fetch(`${API_BASE}/all-clients`);
